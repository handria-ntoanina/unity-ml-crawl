...
class Storage():
    ...
    def new(network, state_size, action_size, num_agents,device, noise, seed, memory_size=int(1e5),
        ...
        memory = ReplayBuffer(action_size, device, memory_size, buffer_size, seed)
    ...
    @staticmethod
    def save(filename, agent: MADDPG):
    ...
    def load(filename, device, num_agents) -> MADDPG:
        ...
        agent = MADDPG(network_local,network_target,num_agents,checkpoint['memory'],device, checkpoint['GRADIENT_CLIP'],
